...
`sh "${CLAUDE_SKILL_DIR}/install.sh" $ARGUMENTS` を実行する。引数が無ければ pre-push hook を `~/.gitconfig` の `hook.never-unsigned-commit` に登録し、`--uninstall` なら解除する。どちらもユーザーのグローバルな git 設定を書き換えるが、このスキルを呼んだこと自体を実行の依頼として扱ってよい。それ以外の引数ではスクリプトが使い方を表示して失敗するので、その出力を伝える。
