...
`${CLAUDE_SKILL_DIR}/install.sh` を実行して、pre-push hook を `~/.gitconfig` の `hook.never-meaningless-branch-name` に登録する。解除を依頼されたときは `--uninstall` を付けて実行する。どちらもユーザーのグローバルな git 設定を書き換えるが、このスキルを呼んだこと自体を実行の依頼として扱ってよい。
